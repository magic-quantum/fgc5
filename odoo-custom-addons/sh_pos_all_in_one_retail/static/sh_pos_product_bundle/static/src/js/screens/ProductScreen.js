                    ...
                    if (confirmed) {
                        ...
                        await _.each($("#bundle_product_table").find("tr.data_tr"),async function (row) {
                            for (var i=0; i<$(row).find("input.qty_input").length;i++){
                                ...
                                product_options["quantity"] = parseFloat($($input).val());
                                product_options["is_bundled"] = true;
                                product_options["sh_main_product_id"] = product_id;
                                const sh_product = self.env.pos.db.product_by_id[$(row).data("id")];
                                await self.env.pos.get_order().add_product(sh_product, product_options);
                                let lines = self.env.pos.get_order().get_orderlines();
                                for (var j = 0; j < lines.length; j++) {
                                    console.log("lines >>> ",lines[j]['sh_main_product_id'])
                                    if (lines[j].get_product().id === sh_product.id && lines[j]["is_bundled"] && lines[j]['sh_main_product_id'] === product_id) {
                                        lines[j].set_unit_price(0.0);
                                        lines[j].price_manually_set = true;
                                        lines[j].is_bundled =true;
                                    }
                                }
                            }
                        });
                        ...
                        let main_product_options = {};
                        main_product_options["quantity"] = input_qty;
                        main_product_options["price"] = lst_price;
                        main_product_options["is_bundled"] = false;
            
                        await self.env.pos.get_order().add_product(main_product, main_product_options);
                        let lines = self.env.pos.get_order().get_orderlines();
