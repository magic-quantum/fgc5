							...
							if (notif.payload && notif.payload[0]) {
								...
								if (self && self.env && self.env.pos) {
									if (self.env.pos.config.sh_product_upate == 'online') {
										...
										// remove same elemetn from category product.
										var category_by_id = self.env.pos.db.product_by_category_id
										for (let key in category_by_id) {
											if (category_by_id.hasOwnProperty(key)) {
												category_by_id[key] = [...new Set(category_by_id[key])];
											}
										}
