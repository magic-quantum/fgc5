...
class PartnerUpdate(models.Model):
    ...
    def broadcast_partner(self, partner):
        if partner.id:
            ...
            pos_settle_due_app = self.env['ir.module.module'].sudo().search(
            [('name', '=', 'pos_settle_due')], limit=1)
            if pos_settle_due_app and pos_settle_due_app.state == "installed" and self.user_has_groups('account.group_account_readonly'):
                fields.append("total_due")
            ...
            if 'customer_wallet_count' in self.env['res.partner']._fields:
                fields.append('customer_wallet_count')
            if 'sh_wallet_id' in self.env['res.partner']._fields:
                fields.append('sh_wallet_id')
            if 'sh_customer_wallet' in self.env['res.partner']._fields:
                fields.append('sh_customer_wallet')
